...
закрывает день (main.js шлёт lastFullDayAt=now, а этот скрипт видит
days_missed==0 и сбрасывает last_warned_tier/pending_penalty).
...
def main():
    ...
    for p in players:
        ...
        if hours_missed < TIERS[0][1]:
            # день ещё не считается пропущенным — если до этого был штраф/предупреждение, сбрасываем
            if last_tier:
                try:
        ...
        new_tier = last_tier
        new_penalty = float(p.get("pending_penalty") or 0)
        ...
        if new_tier != last_tier:
            try:
                _patch_player(device_id, new_tier, new_penalty)
            except requests.RequestException:
